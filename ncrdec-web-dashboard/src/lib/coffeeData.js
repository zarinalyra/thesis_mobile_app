...
const WITH_TREE_COLUMNS_SELECT =
...
const ANALYSIS_SELECT =
  "id, tree_id, farm_id, geotag_id, inspection_date, diseases_detected, pests_detected, confidence, chlorosis_readings, per_image_results, image_ids, created_at";
...
async function fetchGeotags({ farmId } = {}) {
  const attempts = [
    () =>
      supabase
        .from("geotags")
        .select(WITH_TREE_COLUMNS_SELECT)
...
function buildLatestTrees(rows) {
  ...
  for (const row of rows || []) {
    ...
    const current = {
      ...
      treeType: String(
        row.tree_type || rawExif.tree_type || rawExif.treeType || "Unknown",
      ),
      datePlanted: String(
        row.date_planted || rawExif.date_planted || rawExif.datePlanted || "-",
      ),
      capturedAt,
      hasDisease,
    ...
    if (
      !existing ||
      toDateTime(current.capturedAt) >= toDateTime(existing.capturedAt)
    ) {
      latestByTree.set(mapKey, current);
    }
  }

  return Array.from(latestByTree.values()).sort(
    (a, b) => toDateTime(b.capturedAt) - toDateTime(a.capturedAt),
  );
}
...
function buildLatestAnalysisByTree(analysisRows) {
  ...
  for (const row of analysisRows || []) {
    ...
    if (
      !existing ||
      toDateTime(row.created_at || row.inspection_date) >=
        toDateTime(existing.created_at || existing.inspection_date)
    ) {
...
export async function loadDashboardData() {
  ...
  const analysisRows = await fetchAnalysisRows(
    latestTrees.map((tree) => tree.treeId),
  );
...
export async function loadRecordsData() {
  const geotagRows = await fetchGeotags();
  const latestTrees = buildLatestTrees(geotagRows);
  const latestTreeById = new Map(
    latestTrees.map((tree) => [String(tree.treeId || ""), tree]),
  );
  const analysisRows = await fetchAnalysisRows(
    latestTrees.map((tree) => tree.treeId),
  );
  ...
  for (const [treeId, rows] of analysisByTreeId.entries()) {
    rows.sort(
      (a, b) =>
        toDateTime(b.created_at || b.inspection_date) -
        toDateTime(a.created_at || a.inspection_date),
    );
  ...
  for (const row of geotagRows) {
    ...
    const linkedByDate =
      (analysisByTreeId.get(treeId) || []).find((analysis) => {
        return (
          toDateKey(analysis.inspection_date || analysis.created_at) ===
          toDateKey(row.captured_at)
        );
      }) || null;
    ...
    const eventImageIds =
      Array.isArray(monitoringAnalysis?.image_ids) &&
      monitoringAnalysis.image_ids.length > 0
    ...
    const imageUrls = eventImageIds
      ...
      .map(
        (image) =>
          supabase.storage.from("leafImages").getPublicUrl(image.file_path).data
            .publicUrl,
      );
  ...
  for (const [treeId, rows] of analysisByTreeId.entries()) {
    ...
    console.log(
      `Processing tree ${treeId}: has ${rows.length} analyses, history currently has ${treeHistory.length} events`,
    );

    for (const analysis of rows) {
      console.log(
        `  Checking analysis ${analysis.id} (date: ${analysis.inspection_date || analysis.created_at})`,
      );
      ...
      const eventImageIds = Array.isArray(analysis?.image_ids)
        ? analysis.image_ids
        : [];
      const imageUrls = eventImageIds
        .map((imageId) => imageById.get(imageId))
    ...
    treeHistory.sort(
      (a, b) =>
        toDateTime(
          b.analysis?.created_at || b.analysis?.inspection_date || b.capturedAt,
        ) -
        toDateTime(
          a.analysis?.created_at || a.analysis?.inspection_date || a.capturedAt,
        ),
    );
    historyByTreeKey.set(tree.treeKey, treeHistory);
    console.log(
      `  Final history for tree ${treeId}: ${treeHistory.length} events`,
    );
  }
...
export async function loadMapMarkers() {
  ...
  const latestTrees = buildLatestTrees(geotagRows);
  const allImageIds = Array.from(
    new Set(latestTrees.flatMap((tree) => tree.imageIds).filter(Boolean)),
  );
  const imageById = await fetchImageUrls(allImageIds);
  const analysisRows = await fetchAnalysisRows(
    latestTrees.map((tree) => tree.treeId),
  ...
  return latestTrees.map((tree) => {
    const latestImages = tree.imageIds
      ...
      .sort(
        (a, b) =>
          toDateTime(b.uploaded_at || 0) - toDateTime(a.uploaded_at || 0),
      )
      .map(
        (image) =>
          supabase.storage.from("leafImages").getPublicUrl(image.file_path).data
