...
function averageExifData(photos: PhotoWithExif[]): {
  ...
  for (const photo of photos) {
    ...
    if (
      photo.exif.accuracy &&
      photo.exif.accuracy > ACCURACY_THRESHOLD_METERS
    ) {
      console.warn(
        `Skipping photo — GPS accuracy too low: ${photo.exif.accuracy}m`,
      );
      continue;
    }
  ...
  return {
    ...
    rawExif: {
      ...
      model: model,
      avgWidth: Math.round(
        photos.reduce((sum, p) => sum + (p.exif.width || 0), 0) / photos.length,
      ),
...
export async function uploadPhotosToSupabase(
  ...
  try {
    ...
    for (let i = 0; i < photos.length; i++) {
      ...
      if (uploadError) {
        throw new Error(
          `Storage upload error for image ${i + 1}: ${uploadError.message}`,
        );
      }
    ...
    console.log("Averaged location:", {
      lat: avgData.latitude,
      lon: avgData.longitude,
      alt: avgData.altitude,
    });
...
// ─────────────────────────────────────────────────────────────
// UPDATE CARD — upload images only, no geotag recomputation
// Returns image IDs so they can be stored in analysis_results
// ─────────────────────────────────────────────────────────────
export async function uploadImagesOnlyToSupabase(
  supabase: any,
  photos: PhotoWithExif[],
  farmId: string,
): Promise<number[]> {
  console.log("=== UPDATE UPLOAD START ===");
  console.log("Uploading", photos.length, "photos (images only, no geotag)");
  ...
  for (let i = 0; i < photos.length; i++) {
    ...
    if (!fileBase64) {
      throw new Error(`Failed to read image ${i + 1}`);
    }
    ...
    console.log(
      `  [${i + 1}/${photos.length}] Uploading to Supabase storage...`,
    );
    const { error: uploadError } = await supabase.storage
      .from("leafImages")
      .upload(filePath, bytes, {
    ...
    // Insert image record and get the ID back
    const { data: imageData, error: imageError } = await supabase
      .from("images")
      .insert({ file_path: filePath })
      .select()
    ...
    if (imageError) {
      console.warn(`Image record insert warning: ${imageError.message}`);
    } else {
      imageIds.push(imageData.id);
    }

    console.log(
      `  [${i + 1}/${photos.length}] ✓ Uploaded, image ID: ${imageData?.id}`,
    );
  }

  console.log("Image IDs from update:", imageIds);
  console.log("✅ Update upload complete!");
  console.log("=== UPDATE UPLOAD END ===");
  return imageIds;
}
