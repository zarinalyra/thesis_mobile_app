...
function averageExifData(photos: PhotoWithExif[]): {
  ...
  for (const photo of photos) {
    ...
    // If accuracy is available, weight = 1/accuracy. Otherwise weight = 1 (equal weight)
    const weight = photo.exif.accuracy ? 1 / photo.exif.accuracy : 1;
    ...
    if (alt != null) {
      const altWeight = photo.exif.accuracy ? 1 / photo.exif.accuracy : 1;
      altSum += alt * altWeight;
  ...
  const avgLatitude = latSum / weightSum;
  const avgLongitude = lonSum / weightSum;
  const avgAltitude = altWeightSum > 0 ? altSum / altWeightSum : null;
  ...
  return {
    ...
    rawExif: {
      ...
      validPhotoCount: Math.round(weightSum), // approximate
      make,
      model,
      avgWidth: Math.round(
